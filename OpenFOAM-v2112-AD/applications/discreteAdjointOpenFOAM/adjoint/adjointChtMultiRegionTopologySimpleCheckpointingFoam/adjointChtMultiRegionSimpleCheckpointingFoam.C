...
int main(int argc, char *argv[])
{
    ...
    forAll(alphaFluid,i){
        ADmode::global_tape->register_variable(alphaFluid[i].begin(),alphaFluid[i].end());
    }
    ...
    std::function<bool()> runStep = [&]() -> bool {
        ...
        if(optZoneID >= 0 && (runTime.timeIndex() % 10) == 0){
            ...
            Info << "OPT" << endl;
            for(auto cI : optZone){
                dco::value(alphaFluid[0][cI]) = AD::passiveValue(alpha_range.clip(alphaFluid[0][cI] - 0.1*sensFluid[0][cI]));
            }
        }
    ...
    runTime.setTime(2000,label(2000));
    forAll(alphaFluid,i){
        forAll(alphaFluid[i],j){
            sensFluid[i][j] = AD::derivative(alphaFluid[i][j]);
        }
        sensFluid[i].write();
    }
