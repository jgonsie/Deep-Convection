...
class Checkpoint
{
	...
	std::vector <int> ch;
	std::vector <int> ord_ch;
	std::vector <int> number_of_writes;
	std::vector <int> number_of_reads;
	void print_ch(ostream &file) { file << std::endl; for(int i=0;i<snaps;i++) file << ch[i] << " "; }
	void print_ord_ch(ostream &file) { file << std::endl; for(int i=0;i<snaps;i++) file << ord_ch[i] << " "; }
...
class Offline : public Schedule
{
	...
	std::vector <int> get_num_ch() { return num_ch; }
...
class Revolve
{
	...
	std::vector <int> get_write_and_read_counts();
	...
	void number_of_writes() { cout << std::endl; for(int i=0;i<snaps;i++) cout << checkpoint->number_of_writes[i] << " "; }
	void number_of_reads() { cout << std::endl; for(int i=0;i<snaps;i++) cout << checkpoint->number_of_reads[i] << " "; }
	...
	std::vector <int> indizes_ram,indizes_rom;
};
