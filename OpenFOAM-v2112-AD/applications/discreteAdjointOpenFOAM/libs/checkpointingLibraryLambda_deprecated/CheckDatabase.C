...
void CheckDatabase::restoreCheckpoint(int id)
{
    ...
    //Info << "Restoring: " << id << " " << checkTimes[id].second << " " << checkTimes[id].first << endl;
...
void CheckDatabase::addDictionaryCheckpoint(Foam::dictionary& d){
    addCheckObject(new CheckObjectDictionary(d));
}
