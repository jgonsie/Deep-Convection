...
CheckMethod::CheckMethod(CheckDatabase& cd, CheckDict& checkDict, Foam::Time& runTime)
    : checkData(cd), runTime(runTime)
{
    ...
    nCheckpoints = Foam::readInt(checkDict.checkpointSettings("nCheckpoints"));
    nTapeSteps = Foam::readInt(checkDict.checkpointSettings("nTapeSteps"));
}
