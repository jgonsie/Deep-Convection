...
int Schedule::numforw(int steps, int snaps)
{
	...
	if (snaps < 1)
	{
		Foam::Info << " error occurs in numforw: snaps < 1 " << Foam::endl ;
		return -1;
	}
	if (snaps > checkup)
	{
		Foam::Info << " number of snaps = " << snaps << " exceeds checkup " << Foam::endl;
		Foam::Info << " redefine 'checkup'" << Foam::endl;
		return -1;
	}
	reps = 0;
	range = 1;
	...
	Foam::Info << " range = " << range << " reps= " <<  reps << Foam::endl;
	if (reps > repsup)
	{
		Foam::Info << " number of reps = " << reps << " exceeds repsup " << Foam::endl;
		Foam::Info << " redefine 'repsup' " << Foam::endl;
		return -1;
	}
...
Online::Online(int sn,Checkpoint *c,bool o=false) : Schedule(sn,c)
{
	...
	//Foam::Info <<"\n Vectors size = " << checkpoint->ord_ch.size() << Foam::endl;
	//for(int i=0;i<snaps;i++) Foam::Info << " " << checkpoint->ord_ch[i];
	output=o;
...
{
	//Foam::Info <<" check = "<< check << " ch[check] " << checkpoint->ch[check]<< " capo " << capo << Foam::endl; 
	checkpoint->commands++;
		...
		// condition for takeshot for r=1
	{
		...
		/*   	  Foam::Info <<" 2 *check %d ch[*check] %d *capo %d Foam::endl",*check,ch[*check],*capo); */
			...
			// condition for advance for r=2
		{
			switch (snaps)
			{
				...
				default: if (capo == snaps-1)
					...
					if (output)
						Foam::Info << " iter " << iter << " incr " << incr << "offset" << offset << Foam::endl;
					if (t == 0)
					{
						if (iter < offset)
						{
					...
					if (output)
						Foam::Info << " iter " << iter << "incr " << incr << Foam::endl;
					Foam::Info << " not implemented yet" << Foam::endl;
					return ACTION::error;
			}
		}
		else
			// takeshot for r=2
		{
			switch (snaps)
			{
				...
				case 3: checkpoint->ch[ind] = capo;
					check = ind;
					Foam::Info <<" iter " << iter << " num_rep[1] " << num_rep[1] << Foam::endl;
					if (iter == num_rep[1])
					{
						iter = 0;
						t++;
				...
				default: if (capo < snaps+2)
					{
						...
						if (capo == snaps+1)
						{
							...
							if (output)
								Foam::Info << " oldind " << oldind << " ind " << ind << Foam::endl;
							...
							if (output)
							{
								Foam::Info << " ind " << ind << " incr " << incr << " iter " << iter << Foam::endl;
								for(int j=0;j<snaps;j++)
									Foam::Info << " j " << j << " ord_ch " << checkpoint->ord_ch[j] << " ch " << checkpoint->ch[checkpoint->ord_ch[j]] << " rep " << num_rep[checkpoint->ord_ch[j]] << Foam::endl;
							}
						}
						// Increase the number of takeshots and the corresponding checkpoint
						checkpoint->takeshots++;
					...
					{
						if (output)
							Foam::Info <<" ind " << ind << " incr " <<  incr << " iter " << iter << " offset " << offset << Foam::endl;
						if (iter == offset)
						{
							offset--;
							iter = 1;
						...
						else
						{
							...
							ind++;
							if (output)
								Foam::Info << " xx ind " << ind << " incr " << incr << " iter " << iter << Foam::endl;
						}
						// Increase the number of takeshots and the corresponding checkpoint
						checkpoint->takeshots++;
						//checkpoint->number_of_writes[check]++;
...
int Arevolve::tmin(int steps, int snaps)
{
	...
	if (snaps < 1)
	{
		Foam::Info << " error occurs in tmin: snaps < 1 " << Foam::endl;
		return -1;
	}
	if (snaps > checkup)
	{
...
{
	int index;
	//checkpoint->print_ch(Foam::Info);
	//checkpoint->print_ord_ch(Foam::Info);
	...
	else
	{
		...
		{
			//Foam::Info << "is_dispensable " << Foam::endl;
		...
		{
			//Foam::Info << "is_d=true " << Foam::endl;
			checkpoint->ch[index_old] = capo;
			check=index_old;
			lmin = get_lmin();
			l[index_old] = lmin+1;
			d[index_old]=false;
			//Foam::Info << "check = " << check << "  forward= " << lmin+1 << Foam::endl;
			adjust_cp(index_old);
			is_d=false;
			forward=1;
			checkpoint->takeshots++;
...
Offline::Offline( int sn,Checkpoint *c,Online *o,int f) : Schedule(sn,c )
{
	...
	for(int i=0;i<snaps;i++)
	{
		...
		if (o->get_output())
			Foam::Info << " i " << i << " num_ch " << num_ch[i] << " ch " << checkpoint->ch[i] << Foam::endl;
		...
		case 0:   /* reduce capo to previous checkpoint, unless done  */
			if (check == -1 || capo==checkpoint->ch[0] )
			{
				if (info > 0)
				{
					...
					Foam::Info << "\n commands: " /*<< setw(5)*/ << checkpoint->commands << Foam::endl;
				}
				return ACTION::terminate;
			}
			else
		...
		default:
				...
				// Initialisation
			{
				...
				if (info > 0)
				{
					...
					Foam::Info << " prediction of needed forward steps: " /*<< setw(8)*/ << num << " => " << Foam::endl;
					Foam::Info << " slowdown factor: " /*<< setiosflags(ios::fixed) << setprecision(4)*/ 
					           << static_cast<double>(num)/(fine-capo) << Foam::endl << Foam::endl;
				}
				oldfine = fine;
...
void Revolve::turn(int final)
{
	if(online)
	{
		fine=final;
		capo=final-1;
		Online *g = new Online(static_cast<Online &>(*f));
		delete f;
...
double Revolve::expense(int steps, int snaps)
{
	...
	if (snaps < 1)
	{
		Foam::Info << " error occurs in expense: snaps < 0 " << Foam::endl;
		return -1;
	}
	if (steps < 1)
	{
		Foam::Info <<" error occurs in expense: steps < 0 " << Foam::endl;
		return -1;
	}
	ratio = numforw(steps,snaps);
	if (ratio == -1)
		return -1;
	ratio = ratio/steps;
	return ratio;
...
int Revolve::maxrange(int ss, int tt)
{
	...
	if ((tt<0) || (ss<0))
	{
		Foam::Info << " error in MAXRANGE: negative parameter ";
		return -1;
	}
	for (i=1; i<= tt; i++)
	{
		...
		{
			ires=MAXINT;
			Foam::Info << " warning from MAXRANGE: returned maximal integer "<< ires << Foam::endl;
			return ires;
		}
	}
...
int cal(int l,int c,int i)
{
	...
	return static_cast<int>(floor(0.5*(1+2*c)-sqrt(pow(0.5*(1+2*c),2)+2*i+4-2*l))-1);
}
...
int Revolve::get_number_of_writes_i(int l, int c,int i)
{
	...
	else if(l <= (1+i)*c-0.5*(i-1)*i+1) return static_cast<int>(floor(0.5*(1+2*c)-sqrt(pow(0.5*(1+2*c),2)+2*i+4-2*l)));
	...
	else
	{
		...
		if (a==0)	
			...
			k = static_cast<int>(floor(c-pow(2./(a+sqrt(a*a-108.)),1./3.)-1./3.*pow(0.5*(a+sqrt(a*a-108.)),1./3.)));
		
		double l_k = 1./6.*k*k*k-c/2.*k*k+1./6.*(3.*c*c-1.)*k+l_0;
		
		if(i<=k)
			return static_cast<int>(1./2.*i*i+3./2.*i+1.);
		else    
		{
			int w_i_k= static_cast<int>(i*k+i+1.- 1./2.*k*(k-1.));
			return w_i_k+cal(l-static_cast<int>(l_k)+2*(c-k)+1,c-k,i-k);
		}
	}
}
...
int Revolve::get_number_of_reads_i(int l, int c,int i)
{
	...
	{	
		...
		if(l >= c*c+2*c+1-c*i+0.5*(i*i-i))
			return static_cast<int>(floor(a+sqrt(a*a-b+2.*(l-0.5*c*c-1.5*c-1))));
		else
			return i+2;
	}
