...
) const
{
    ...
    if (firsttimein)
    {
        ...
        if (f)
        {
            ...
            {
                *(iter++) = AD::passiveValue(v.x());
                *(iter++) = AD::passiveValue(v.y());
                *(iter++) = AD::passiveValue(v.z());
            }
...
) const
{
    ...
    (
        ksp,
        AD::passiveValue(relTol_),
        AD::passiveValue(tolerance_),
        PETSC_DEFAULT,
        maxIter_
    );
}
...
) const
{
    ...
    if (f)
    {
       ...
       if (nProcValues)
       {
          ...
          forAll(interfaces, patchi)
          {
             ...
             if (intf)
             {
                ...
                forAll(faceCells, i)
                {
                   *ptr_v++ = AD::passiveValue(-bCoeffs[i]);
                }
             }
          }
       ...
       // The diagonal
       {
           ...
           {
               PetscScalar val = AD::passiveValue(diagVal[celli]);
       ...
       for (label idx=0; idx < nrows_; ++idx)
       {
           if (lowNonZero[idx] > 0)
           {
               ...
               for (label jidx=0; jidx<lowNonZero[idx]; ++jidx)
               {
                   ...
                   globVals[jidx] = AD::passiveValue(uppVal[kidx]);
                   ++kidx;
               }
       ...
       if (!is_sbaij(Amat))
       {
           ...
           for (label idx=0; idx < nIntFaces_; ++idx)
           {
               ...
               PetscScalar val = AD::passiveValue(lowVal[idx]);
           ...
           forAll(interfaces, patchi)
           {
               if (interfaces.set(patchi))
               {
                   ...
                   forAll(faceCells, i)
                   {
                       ...
                       PetscScalar val = AD::passiveValue(-bCoeffs[i]);
