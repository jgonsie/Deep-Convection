...
) const
{
    ...
    if (step.first)
    {
        ...
        // change for discreteAdjointOpenFOAM by Towara@STCE: use std::max,min
        kTarg_ = std::max(label(1), std::min(label(kMaxx_ - 1), label(AD::passiveValue(logTol))));
    }
    ...
    {
        // change for discreteAdjointOpenFOAM by Towara@STCE: cast to scalar
    ...
    // change for discreteAdjointOpenFOAM by Towara@STCE: cast to scalar
    step.dxTry = step.forward ? dxNew : scalar(-dxNew);
}
