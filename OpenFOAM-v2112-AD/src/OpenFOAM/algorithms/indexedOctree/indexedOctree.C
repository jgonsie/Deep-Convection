    ...
    nodeTypes_(0)
{
    ...
    DynamicList<node> nodes(label(shapes.size() / AD::passiveValue(maxLeafRatio)));
    // change for discreteAdjointOpenFOAM by Towara@STCE: cast to passive
    DynamicList<labelList> contents(label(shapes.size() / AD::passiveValue(maxLeafRatio)));
    ...
    for (; nLevels < maxLevels; nLevels++)
    {
        ...
        splitNodes
        (
            label(AD::passiveValue(maxLeafRatio)),
            nodes,
            contents
        );
