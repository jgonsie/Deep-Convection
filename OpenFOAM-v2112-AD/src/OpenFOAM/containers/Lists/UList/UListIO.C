...
) const
{
    ...
    if (os.format() == IOstream::BINARY && is_contiguous<T>::value)
    {
        ...
        if (len)
        {
            ...
            if(!contains_active_type<T>::value){
                os.write(list.cdata_bytes(), list.size_bytes());
            }else{
                const label nBytes = len*sizeof(T)*sizeof(double)/sizeof(scalar);
                const label nDoubles = len*sizeof(T)/sizeof(scalar);
                std::vector<scalarPassive> buf(nDoubles);
                for(label i=0;i<nDoubles;i++){
    		        buf[i] = AD::passiveValue(reinterpret_cast<const scalar*>(list.cdata())[i]);
                }
                os.write(reinterpret_cast<const char*>(buf.data()),nBytes);
            }
        }
    }
        ...
        if (is.format() == IOstream::BINARY && is_contiguous<T>::value)
        {
            ...
            if (len)
            {
                if(!contains_active_type<T>::value){
                    ...
                    const label nDoubles = len*sizeof(T)/sizeof(scalar);
                    std::vector<scalarPassive> buf(nDoubles);
                    is.read(reinterpret_cast<char*>(buf.data()), nBytes);
                    for(label i=0;i<nDoubles;i++){
    		            reinterpret_cast<scalar*>(list.data())[i] = buf[i];
                    }
                }
