...
// Towara: cast to passive
const int Foam::Time::maxPrecision_(AD::passiveValue(3 - log10(SMALL)));
...
void Foam::Time::adjustDeltaT()
{
    ...
    if (adjustTime)
    {
        ...
        if (nSteps < scalar(labelMax))
        {
            ...
            label nStepsToNextWrite = max(1, round(AD::passiveValue(nSteps)));
...
bool Foam::Time::stopAt(const stopAtControls stopCtrl) const
{
    ...
    // Towara: reset writeTime_, needed for Checkpointing
    writeTime_ = false;
...
Foam::Time& Foam::Time::operator++()
{
    ...
    if (!subCycling_)
    {
        ...
        switch (writeControl_)
        {
            ...
            case wcTimeStep:
                // change for discreteAdjointOpenFOAM by Towara@STCE: cast to passive
                writeTime_ = !(timeIndex_ % label(AD::passiveValue(writeInterval_)));
            break;
            ...
            case wcAdjustableRunTime:
            {
                ...
                (
                    AD::passiveValue(((value() - startTime_) + 0.5*deltaT_)
                  / AD::passiveValue(writeInterval_))
                );
            ...
            {
                // change for discreteAdjointOpenFOAM by Towara@STCE: cast to passive
                // change for discreteAdjointOpenFOAM by Towara@STCE: change double to doubleScalar
                const label writeIndex = label
                (
                    AD::passiveValue(returnReduce(static_cast<doubleScalar>(elapsedCpuTime()), maxOp<doubleScalar>()))
                  / AD::passiveValue(writeInterval_)
                );
                if (writeIndex > writeTimeIndex_)
                {
...
// change for discreteAdjointOpenFOAM by Towara@STCE: cast to passive
                const label writeIndex = label
                (
                    AD::passiveValue( returnReduce(scalar(elapsedClockTime()), maxOp<scalar>()) / writeInterval_ )
                );
                if (writeIndex > writeTimeIndex_)
                {
                    writeTime_ = true;
