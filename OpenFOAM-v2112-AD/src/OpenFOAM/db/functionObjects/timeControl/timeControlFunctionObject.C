...
)
{
    ...
    for (label iter = 0; iter < 100; iter++)
    {
        ...
        scalar dfdratio = (y-1)*n*pow(ratio, n-1)-y*scalar(n-1)*pow(ratio, n-2);
    ...
    else
    {
        ...
        for (label iter = 0; iter < 100; iter++)
        {
            ...
            else
            {
                ...
                scalar jumpError =
                   remainder(AD::passiveValue(Sn + presentTime), AD::passiveValue(wantedDT)) / wantedDT;

                if (mag(jumpError) > ROOTSMALL)
                {
                    requiredSteps = label(AD::passiveValue(timeToNextWrite/wantedDT));
    ...
    if (active())
    {
        ...
        )
        {
            ...
            scalar intervalError =
                remainder(AD::passiveValue(writeInterval), AD::passiveValue(wantedDT))/writeInterval;
            if
            (
