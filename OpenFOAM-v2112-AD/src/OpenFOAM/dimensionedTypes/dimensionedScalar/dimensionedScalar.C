...
// Towara: active passive overloads
dimensionedScalar operator*(const dimensionedScalar& ds1, const scalarPassive s2)
{
    return ds1 * dimensionedScalar(scalar(s2));
}
dimensionedScalar operator*(const scalarPassive s1, const dimensionedScalar& ds2)
{
    return dimensionedScalar(scalar(s1)) * ds2;
}
...
// change for discreteAdjointOpenFOAM by Towara@STCE: change namespace for transFunc
...
transFunc(erf)
transFunc(erfc)
// change for discreteAdjointOpenFOAM by Towara@STCE: remove transFuncs
/*
transFunc(lgamma)
transFunc(j0)
transFunc(j1)
