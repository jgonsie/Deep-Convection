...
) const
{
    if (sizeof(scalar) != sizeof(float) && Pstream::floatTransfer && f.size())
    {
        ...
        {
            // change for discreteAdjointOpenFOAM by Towara@STCE: cast to passive float (danger of losing active information!)
            fArray[i] = AD::passiveValue(sArray[i] - slast[i%nCmpts]);
        }
