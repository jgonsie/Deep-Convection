    ...
    oldCellCentresPtr_(nullptr)
{
    ...
    if(AD::isTapeActive()){
        Info << "Registering Nodes" << endl;
        forAll(points_,i){
            AD::registerInputVariable(points_[i][0]);
            AD::registerInputVariable(points_[i][1]);
            AD::registerInputVariable(points_[i][2]);
        }
    }
#endif
    ...
    // Towara: Surface sensitivities
    /*if(AD::isTapeActive()){
        forAll(boundary_,patchI){
            Info << "Registering " << boundary_.names()[patchI] << endl;
            forAll(boundary_[patchI].faceCells(),faceJ){
                label k = boundary_[patchI].faceCells()[faceJ];
                //Info << patchI << " " << faceJ << " " << k << endl;
                forAll(this->cellPoints()[k],j){
                    label tmp = this->cellPoints()[k][j];
                    //Info << patchI << " " << faceJ << " " << k << " " << tmp << endl;
                    AD::registerInputVariable(points_[tmp][0]);
                    AD::registerInputVariable(points_[tmp][1]);
                    AD::registerInputVariable(points_[tmp][2]);
                }
            }
        }
    }*/
