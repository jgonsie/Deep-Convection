...
{
    return round(AD::passiveValue(scalar01()));
}
...
{
    return round(AD::passiveValue(GaussNormal<scalar>()));
}
...
Foam::label Foam::Random::position(const label& start, const label& end)
{
    ...
    const label val = start + label(AD::passiveValue(scalar01()*(end - start + 1)));
...
Foam::label Foam::Random::globalSample01()
{
    ...
    if (Pstream::master())
    {
        value = round(AD::passiveValue(scalar01()));
    }

    Pstream::scatter(value);

    return AD::passiveValue(value);
}
