...
) const
{
    ...
    return fieldDict.getOrDefault("min", Foam::scalar(-GREAT));
}
        ...
        forAll(valueSets, fieldi)
        {
            ...
            for (direction cmpti=0; cmpti < pTraits<Type>::nComponents; ++cmpti)
            {
                ...
                forAll(field, i)
                {
                    ...
                    float f = AD::passiveValue(component(v, cmpti));
                    float minf = AD::passiveValue(max(component(minValue, cmpti), minValueLimit));
                    float maxf = AD::passiveValue(min(component(maxValue, cmpti), maxValueLimit));
                    float deltaf = AD::passiveValue(maxf - minf + SMALL);
            ...
            forAll(valueSets, fieldi)
            {
                ...
                )
                {
                    ...
                    forAll(field, i)
                    {
                        ...
                        float f = AD::passiveValue(component(v, cmpti));
                        float minf =
                            AD::passiveValue(max(component(minValue, cmpti), minValueLimit));
                        float maxf =
                            AD::passiveValue(min(component(maxValue, cmpti), maxValueLimit));
                        float deltaf = AD::passiveValue(maxf - minf + SMALL);
