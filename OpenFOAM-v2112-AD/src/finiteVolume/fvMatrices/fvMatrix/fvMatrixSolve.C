...
#include "PrecisionAdaptor.H"
    ...
    #include "fvMatrixSolveFillTapeGapI.H"
#endif
...
)
{
    ...
    for (direction cmpt=0; cmpt<Type::nComponents; cmpt++)
    {
        ...
        if(tapeWasActive && useSDLS){
            ...
            forAll(psi, i){
                AD::registerInputVariable(psiCmpt[i]);
            }
            forAll(psi.boundaryField(), i){
                if(psi.boundaryField().types()[i] == "processor"){
                    forAll(psi.boundaryField()[i], j){
                        AD::registerInputVariable(psi.boundaryField()[i][j][cmpt]);
                    }
                }
            }
            AD::switchTapeToPassive();
            ...
            const volScalarField pC = psi.component(cmpt)();

            // Info << "DEBUG: Tape Idx starts at " << AD::tapeIndex(pC[0]) << endl;

            AD::switchTapeToActive();

            auto callbackLbda = [cmpt,*this,pC,sourceCmpt](){symbolic::fillSolverGap(cmpt,*this,pC,sourceCmpt);};
            //auto foo = [=](){symbolic::fillSolverGap(cmpt,*this,psi.component(cmpt)(),sourceCmpt);};
