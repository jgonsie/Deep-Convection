...
#if (defined(DAOF_AD_MODE_A1S) /*|| defined(DAOF_AD_MODE_T2A1S)*/) && defined(DAOF_SDLS)
    #define ENABLE_SDLS
#endif
// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //
...
)
{
    ...
    {
        Info
            << "SDLS for implicit face treatment is untested" << nl
            << endl;
    }
    bool useSDLS = solverControls.lookupOrDefault<bool>("SDLS",true);
    bool tapeWasActive = AD::isTapeActive();

    if(tapeWasActive && useSDLS)
    {
        AD::switchTapeToPassive();
    }
#endif

    GeometricField<scalar, fvPatchField, volMesh>& psi__ =
    ...
    if(tapeWasActive && useSDLS){
        ...
        AD::registerInputVariable(psi.begin(),psi.end());
        forAll(psi_.boundaryField(),i)
        {
            if(psi_.boundaryField().types()[i] == "processor")
            {
                AD::registerInputVariable(psi_.boundaryField()[i].begin(),psi_.boundaryField()[i].end());
            }
        }
        // capture objects by value in lambda, copy psi_
        auto callbackLbda = [*this, psi=psi_, totalSource](){symbolic::fillSolverGap(-1,*this,psi,totalSource);};
        AD::insertAdjointCallback(std::move(callbackLbda));
    }
#endif
