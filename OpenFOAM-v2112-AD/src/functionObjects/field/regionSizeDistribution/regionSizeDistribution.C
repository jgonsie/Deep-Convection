    ...
    if (allRegionVolume.size())
    {
        ...
        if (isoPlanes_)
        {
            ...
            forAll(distToPlane, i)
            {
                ...
                {
                    downstreamIndices[i] = AD::passiveValue(distToPlane[i]/deltaX);
                }
            }

            scalarField binDownCount(AD::passiveValue(nDownstreamBins_), Zero);
            forAll(distToPlane, i)
            {
            ...
            {
                // Construct mids of bins for plotting
                pointField xBin(AD::passiveValue(nDownstreamBins_));
        ...
        forAll(sortedDiameters, i)
        {
            indices[i] = AD::passiveValue((sortedDiameters[i]-minDiam_)/delta);
        }
