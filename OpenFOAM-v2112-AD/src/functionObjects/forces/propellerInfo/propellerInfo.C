...
{
    label nPoint = nRadius*AD::passiveValue(nTheta);
    if (r1 < SMALL)
    {
    ...
    {
        nPoint += AD::passiveValue(nTheta);
    }
    const label nFace = nRadius*AD::passiveValue(nTheta);
    ...
    const List<label> ptIDs(identity(AD::passiveValue(nTheta)));
    ...
    for (int radiusi = 0; radiusi < nRadius; ++radiusi)
    {
        ...
        else
        {
            for (label thetai = 0; thetai < nTheta; ++thetai)
            {
                ...
                label offset = pointOffset0 + (radiusi-radiusOffset)*AD::passiveValue(nTheta);
                ...
                // Outer
                facePts.append(offset + AD::passiveValue(nTheta) + ptIDs.fcIndex(thetai));
                facePts.append(offset + AD::passiveValue(nTheta) + ptIDs.fcIndex(thetai - 1));
