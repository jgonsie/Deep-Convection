...
)
{
    ...
    {
        point Cc(C[celli] - c0);
        label i = round(AD::passiveValue((Cc.x() - 0.5*deltaC.x())/(deltaC.x())*(N.x() - 1)));
        label j = round(AD::passiveValue((Cc.y() - 0.5*deltaC.y())/(deltaC.y())*(N.y() - 1)));
        label k = round(AD::passiveValue((Cc.z() - 0.5*deltaC.z())/(deltaC.z())*(N.z() - 1)));
        label kappai = round(AD::passiveValue(Foam::sqrt(scalar(i*i + j*j + k*k))));
...
bool Foam::functionObjects::energySpectrum::read(const dictionary& dict)
{
    ...
    N_ = Vector<int>
    (
        round(AD::passiveValue(nCellXYZ.x())),
        round(AD::passiveValue(nCellXYZ.z())),
        round(AD::passiveValue(nCellXYZ.z()))
    );
    ...
    forAll(C, celli)
    {
        label i = round(AD::passiveValue((C[celli].x() - c0_.x())/(deltaC_.x())*(N_.x() - 1)));
        label j = round(AD::passiveValue((C[celli].y() - c0_.y())/(deltaC_.y())*(N_.y() - 1)));
        label k = round(AD::passiveValue((C[celli].z() - c0_.z())/(deltaC_.z())*(N_.z() - 1)));
