...
void Foam::distribution::add(const scalar valueToAdd)
{
    ...
    // change for discreteAdjointOpenFOAM by Towara@STCE: cast to passive label
    label n = label(AD::passiveValue(valueToAdd/binWidth_)) - label(AD::passiveValue(neg(valueToAdd/binWidth_)));
...
)
{
    ...
    (
        AD::passiveValue(lowestOldBin + 0.5*sign(lowestOldBin))
    );
