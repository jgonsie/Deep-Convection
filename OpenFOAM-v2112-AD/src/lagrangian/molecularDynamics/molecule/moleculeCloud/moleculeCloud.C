    ...
    for (const cellZone& zone : cellZones)
    {
        if (zone.size())
        {
            ...
            else
            {
                ...
                point latticeAnchor
                (
                    label(AD::passiveValue(latticeMid.x() + 0.5*sign(latticeMid.x()))),
                    label(AD::passiveValue(latticeMid.y() + 0.5*sign(latticeMid.y()))),
                    label(AD::passiveValue(latticeMid.z() + 0.5*sign(latticeMid.z())))
                );
