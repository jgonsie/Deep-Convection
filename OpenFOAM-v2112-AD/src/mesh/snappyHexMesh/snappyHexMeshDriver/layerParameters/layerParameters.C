...
)
{
    ...
    {
        minR = 0.0;
        maxR = pow(scalar(totalOverFirst/n), scalar(1)/(n-1));
    }
    else
    {
        minR = pow(scalar(totalOverFirst/n), scalar(1)/(n-1));
...
)
{
    switch (layerSpec)
    {
        ...
        case FINAL_AND_EXPANSION:
        {
            ...
            {
                return finalLayerThickness*pow(scalar(1.0/expansionRatio), nLayers-1);
            }
        }
        break;
