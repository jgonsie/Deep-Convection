    ...
    bb_(meshRefiner_.mesh().bounds())
{
    ...
    n_ = labelVector
    (
        round(AD::passiveValue(meshSpan.x()/level0Len)),
        round(AD::passiveValue(meshSpan.y()/level0Len)),
        round(AD::passiveValue(meshSpan.z()/level0Len))
    );
    label nTot = n_.x()*n_.y()*n_.z();
    while (nTot < 1000000)  //1048576)
    {
        //n_ *= 2;
        n_ = { 2*n_[0], 2*n_[1], 2*n_[2] };
        nTot = AD::passiveValue(n_.x()*n_.y()*n_.z());
    }
...
)
{
    ...
    if (Pstream::master())
    {
        ...
        forAll(levelCounts, leveli)
        {
            ...
            cellCount += AD::passiveValue(levelCounts[leveli]*nCellsPerVoxel);
        }
