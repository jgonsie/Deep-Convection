    ...
    )
{
    ...
    if (io.typeHeaderOk<volScalarField>(true))
    {
        ...
        {
            // Round to integer
            cellTypes_[celli] = AD::passiveValue(volCellTypes[celli]);
        }
    }
...
bool Foam::cellCellStencils::inverseDistance::update()
{
...

    {
        ...
        else
        {
            ...
            {
                nDivs = label(AD::passiveValue(Foam::sqrt(scalar(mesh_.nCells()))));
            }
            else
            {
                nDivs = label(AD::passiveValue(Foam::cbrt(scalar(mesh_.nCells()))));
            }
