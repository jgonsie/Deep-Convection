...
)
{
    ...
    const scalar fRatio = pow(2.0, scalar(1.0/octave));
    const scalar fRatioL2C = pow(2.0, scalar(0.5/octave));
...
) const
{
    ...
    for (label bandI = 0; bandI < freqBandIDs.size() - 1; ++bandI)
    {
        ...
        {
            label f0 = AD::passiveValue( f[freqI] );
            label f1 = AD::passiveValue( f[freqI + 1] );
...
{
    return p0*pow(10.0, scalar(db/20.0));
}
