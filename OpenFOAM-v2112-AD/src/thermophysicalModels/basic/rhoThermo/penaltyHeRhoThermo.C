...
)
{
    ...
    forAll(TCells, celli)
    {
        ...
        alphaCells[celli] = penalty_[celli]*scalar(solidKappa_/solidCp_) + scalar(1.0-penalty_[celli]) * mixture_.alphah(pCells[celli], TCells[celli]);
    }
    ...
    forAll(pBf, patchi)
    {
        ...
        if (pT.fixesValue())
        {
            forAll(pT, facei)
            {
                ...
                palpha[facei] = penalty_[facei]*solidKappa_/solidCp_ + scalar(1.0-penalty_[facei])*mixture_.alphah(pp[facei], pT[facei]);
            }
        }
        else
        {
