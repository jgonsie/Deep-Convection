...
void Foam::singleStepReactingMixture<ThermoType>::massAndAirStoichRatios()
{
    ...
    const scalar Wu = this->speciesData()[fuelIndex_].W();
    // change for discreteAdjointOpenFOAM by Towara@STCE: explicitly cast to dimensionedScalar
    stoicRatio_ = static_cast<dimensionedScalar>(
      ...
      * mag(specieStoichCoeffs_[O2Index]))
      / static_cast<dimensionedScalar>(scalar(Wu*mag(specieStoichCoeffs_[fuelIndex_]))));
