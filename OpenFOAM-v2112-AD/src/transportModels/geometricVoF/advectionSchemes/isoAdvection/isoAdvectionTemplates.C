...
)
{
    ...
    for (label n = 0; n < nAlphaBounds_; n++)
    {
        ...
        if (debug)
        {
            ...
            label maxAlphaMinus1 = AD::passiveValue(max(alpha1_.primitiveField() - 1));
            scalar minAlpha = min(alpha1_.primitiveField());
            label nUndershoots = AD::passiveValue(sum(neg0(alpha1_.primitiveField() + aTol)));
            label nOvershoots = AD::passiveValue(sum(pos0(alpha1_.primitiveField() - 1 - aTol)));
...
void Foam::isoAdvection::advect(const SpType& Sp, const SuType& Su)
{
    ...
    DebugInfo
        ...
        << label(AD::passiveValue(100*advectionTime_/(mesh_.time().elapsedCpuTime() + SMALL)))
        << "%" << endl;
