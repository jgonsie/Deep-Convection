...
/* Temporarily taken out
    ...
    if (mag(Un0) > 1e-12) // Note: tolerances
    {
        ...
        {
            scalar value = ((mesh_.points()[AD::passiveValue(fi)] - x0) & n0) / Un0;
            if (mag(value) < 10 * SMALL)
            {
                value = 0;
            }
        ...
        forAll(pTimes_, pi)
        {
            const label oldEdgeSign =
                AD::passiveValue(sign(scalar(pTimes_[(pi + 1) % nPoints] - pTimes_[pi])));
            const label newEdgeSign =
                AD::passiveValue(sign(scalar(pTimes_[(pi + 2) % nPoints] - pTimes_[(pi + 1) % nPoints])));
...
)
{
...

    {
        ...
        {
            const label oldEdgeSign =
                AD::passiveValue( sign(pTimes_[(pi + 1) % nPoints] - pTimes_[pi]) );
            const label newEdgeSign =
                AD::passiveValue( sign(pTimes_[(pi + 2) % nPoints] - pTimes_[(pi + 1) % nPoints]) );
...
)
{
    ...
    else
    {
        ...
        time = 0.0;
        // Note: calcSubFace assumes well-defined 2-point FIIL!!!!
        calcSubFace(faceI, AD::passiveValue(scalar(-sign(Un0))), time);
        initialArea = mag(subFaceArea());
        cutPoints(faceI, time, FIIL);
    }
