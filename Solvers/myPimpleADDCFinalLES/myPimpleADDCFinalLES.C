...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < ncells; i++){
		output_dict[i] = static_cast <double>(AD::value(nut[i]));
		output_dict[i+ncells] = static_cast <double>(AD::value(p[i]));
		output_dict[i+2*ncells] = static_cast <double>(AD::value(U[i][0]));
		output_dict[i+3*ncells] = static_cast <double>(AD::value(U[i][1]));
    }
	
    int iterator = 4*ncells;
	...
	if (patch.name() != "defaultFaces"){
		...
		for(int i = 0; i < nfaces_patch; i++){
			output_dict[i+iterator] = static_cast <double>(AD::value(nut.boundaryField()[patchi][i]));
			output_dict[i+iterator+nfaces_patch] = static_cast <double>(AD::value(p.boundaryField()[patchi][i]));
			output_dict[i+iterator+2*nfaces_patch] = static_cast <double>(AD::value(U.boundaryField()[patchi][i][0]));
			output_dict[i+iterator+3*nfaces_patch] = static_cast <double>(AD::value(U.boundaryField()[patchi][i][1]));
			output_dict[i+iterator+4*nfaces_patch] = static_cast <double>(AD::value(phi.boundaryField()[patchi][i]));
		}
		iterator += 5*nfaces_patch;
		}
    }
    shmdt(input_dict);
    shmdt(output_dict);
